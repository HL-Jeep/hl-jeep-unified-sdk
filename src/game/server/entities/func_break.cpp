...
// Adapted from https://github.com/bacontsu/codename-borea/blob/spirit/spirit-1.8-trinity-op4/dlls/prop_physics.cpp
class CPropPhysics : public CBaseEntity
{
	DECLARE_CLASS(CPropPhysics, CBaseEntity);
	DECLARE_DATAMAP();
...
public:
	...
	void Killed(CBaseEntity* aAttacker, int iGib) override;
	void BounceSound();
	bool KeyValue(KeyValueData* pkvd) override;

	void PropThink();
	void PropTouch(CBaseEntity* pOther);
	void PropUse(CBaseEntity* pActivator, CBaseEntity* pCaller, USE_TYPE useType, float value);
...
BEGIN_DATAMAP(CPropPhysics)
	DEFINE_FUNCTION(PropThink),
	DEFINE_FUNCTION(PropTouch),
	DEFINE_FUNCTION(PropUse),
END_DATAMAP();
...
void CPropPhysics::Spawn()
{
	...
	{
	case 1:
		SetSize(Vector(-26, -26, -26), Vector(26, 26, 26));
		break;

	default:
		SetSize(Vector(-26, 0, -26), Vector(26, 26, 26));
		break;
	}
...
bool CPropPhysics::KeyValue(KeyValueData* pkvd)
{
...
void CPropPhysics::Precache()
{
	...
	m_iShard = PrecacheModel("models/woodgibs.mdl");
	PrecacheSound("weapons/g_bounce1.wav");
	PrecacheSound("weapons/g_bounce2.wav");
	PrecacheSound("weapons/g_bounce3.wav");
}

void CPropPhysics::TraceAttack(CBaseEntity* attacker, float flDamage, Vector vecDir, TraceResult* ptr, int bitsDamageType)
{
	if (flDamage > 0)
	{
		//UTIL_Sparks(Center());

		Vector dir = pev->origin - attacker->pev->origin;
		dir = dir.Normalize();
	...
	return CBaseEntity::TraceAttack(attacker, flDamage, vecDir, ptr, bitsDamageType);
}
...
void CPropPhysics::PropThink()
{
	...
	if (tr.flFraction < 1.0)
	{
		...
		if (m_iPropType == 1)
		{
			...
			if ((int)pev->angles.x == -1)
			{
				pev->angles.x = 0;
				target[0] = 0;
				lerp[0] = 0;
			}
			if ((int)pev->angles.z == -1)
			{
				pev->angles.z = 0;
				target[1] = 0;
				lerp[1] = 0;
...
void CPropPhysics::PropTouch(CBaseEntity* pOther)
{
	...
	if (CBaseEntity::Instance(pevToucher)->ClassnameIs("player") && VARS(pOther->pev->groundentity) != this->pev)
	{
	...
	if (pev->velocity.Length() > 256)
		pOther->TakeDamage(CBaseEntity::Instance(pev), CBaseEntity::Instance(pev), 100, DMG_CLUB);
...
void CPropPhysics::Killed(CBaseEntity* attacker, int iGib)
{
	...
	return CBaseEntity::Killed(attacker, iGib);
}
...
void CPropPhysics::BounceSound()
{
	...
	{
	case 0: this->EmitSound(CHAN_VOICE, "weapons/g_bounce1.wav", 1, ATTN_NORM); break;
	case 1: this->EmitSound(CHAN_VOICE, "weapons/g_bounce2.wav", 1, ATTN_NORM); break;
	case 2: this->EmitSound(CHAN_VOICE, "weapons/g_bounce3.wav", 1, ATTN_NORM); break;
	}
}